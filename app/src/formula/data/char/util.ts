...
export const anomalyMultipliers: Record<AttributeAnomalyKey, number> = {
  fire: 0.5,
  electric: 1.25,
  ether: 0.625,
  ice: 5,
...
export function entriesForChar(data_gen: CharacterDatum): TagMapNodeEntries {
  ...
  return [
    ...
    // Note: count entries (specialty, faction, attribute) are NOT added here.
    // They're supplied by the calculator providers (buildCalculatorEntries,
    // memberAndEquipmentEntries) inside withMember(), which attaches proper
    // `src` tags so the team aggregate can correctly attribute them to each
    ...
    // Lumiflux DMG bonus doesn't exist — skip the listing
    ...(data_gen.attribute !== 'lumiflux'
      ? [
