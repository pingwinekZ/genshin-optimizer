...
/**
 * Re-apply stat filters to existing builds without re-running the solver.
 * Only evaluates filters with qt 'initial' or 'final' (the two
 * "Stat min / max filters" sections on the form), mirroring the solver's
 * constraint math: min keeps read >= value, max keeps read <= value.
 * Returns a filtered list of the same build objects (kept in order).
 */
export async function filterBuildsByStatFilters<
  T extends { discIds: DiscIds; wengineKey?: string },
>(
  builds: T[],
  character: ICachedCharacter,
  team: Team,
  ...
  const activeFilters = filters.filter(
    (f) => !f.disabled && (f.tag.qt === 'initial' || f.tag.qt === 'final')
  )
  if (activeFilters.length === 0) return builds
  ...
  for (let i = 0; i < builds.length; i += BATCH_SIZE) {
    ...
    for (const build of batch) {
      ...
      try {
        ...
        passes = activeFilters.every(({ tag, value, isMax }) => {
          const read = new Read(
            {
              src: character.key,
          ...
          const actual = calc.compute(read).val
          const expected = toDecimal(value, tag.q ?? '')
          return isMax
            ? actual <= expected + EPSILON
            : actual >= expected - EPSILON
        })
      } catch {
        // If stats computation fails, drop the build (matches solver behavior
        // where uncomputable candidates are pruned)
        passes = false
      }

      if (passes) kept.push(build)
    }
  }
...
export function buildCalculatorEntries(
    ...
    .flatMap((teammateDatum: TeammateDatum) => {
      ...
      return [
        ...withMember(
          ...
          ...discEntries,
          // Count entries for team aggregate
  ...
  return [
    ...
    ...withMember(
      ...
      // Count entries for team aggregate
      ownBuff.common.count
        .withSpecialty(allStats.char[character.key].specialty)
        .add(1),
      ownBuff.common.count
        .withFaction(allStats.char[character.key].faction)
        .add(1),
      ownBuff.common.count
        .withTag({ attribute: allStats.char[character.key].attribute })
        .add(1)
    ),
    ...teammateEntries,
