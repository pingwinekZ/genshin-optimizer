...
export function OptimizerGrid({
  ...
  const pinnedTopRowData = useMemo((): EnrichedBuild[] | undefined => {
    ...
    const equippedRow: EnrichedBuild = {
      id: pinnedId,
      index: -1,
    ...
    const pinnedRows: EnrichedBuild[] = pinnedBuilds
      .filter((b) => b.buildId !== pinnedId)
      .map((b) => {
        const enrichedPinned = extEnrichedBuilds?.find(
          (e) => e.id === b.buildId
        )
        return {
          id: b.buildId,
          index: b.index,
          value: enrichedPinned?.value ?? b.value,
          wengineKey: b.wengineKey,
          discIds: enrichedPinned?.discIds ?? b.discIds,
          discSetIds: enrichedPinned?.discSetIds ?? [],
          combatStats: enrichedPinned?.combatStats ?? null,
          baseStats: enrichedPinned?.baseStats ?? null,
        }
      })

    return [equippedRow, ...pinnedRows]
  }, [pinnedBuild, pinnedBuilds, extEnrichedBuilds])
  ...
  // Pinned builds are excluded here — they're shown as pinned top rows.
  const rowData = useMemo(() => {
    if (!builds || builds.length === 0) return []
    const pinnedIds = new Set(pinnedBuilds.map((b) => b.buildId))
    return builds
      .map((build, index) => {
  ...
  }, [builds, extEnrichedBuilds, pinnedBuilds])
  ...
  const columnDefs: ColDef<EnrichedBuild>[] = useMemo(
    ...
    [t, aggregations, statDisplay, specialityKey]
  )
